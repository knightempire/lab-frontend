...
export default function SingleDatePicker({ 
  ...
  const renderCalendar = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      let dayClassName = "flex items-center justify-center cursor-pointer w-8 h-8 text-sm font-medium rounded-md transition-all duration-200 hover:scale-105";
      
      if (isDisabled) {
        dayClassName = "flex items-center justify-center w-8 h-8 text-sm font-medium text-gray-400 rounded-md cursor-not-allowed bg-gray-100 opacity-50";
      } else if (isSelected) {
        dayClassName = "flex items-center justify-center cursor-pointer w-8 h-8 text-sm font-medium text-white rounded-md bg-indigo-500 shadow-md transform scale-105";
      } else if (isToday) {
        dayClassName += " text-indigo-600 border border-indigo-200 font-semibold";
      } else {
        ...
        dayDiv.addEventListener("click", () => {
          ...
          if (daysContainer) {
            daysContainer
              ...
              .forEach((d) => {
                d.classList.remove("bg-indigo-500", "text-white", "shadow-md", "scale-105");
                d.classList.add("text-gray-700");
              });
            dayDiv.classList.add("bg-indigo-500", "text-white", "shadow-md", "scale-105");
  ...
  const inputClassName = `w-full rounded-lg border-2 border-gray-200 bg-white py-2.5 pl-11 pr-11 text-sm text-gray-900 placeholder-gray-400 outline-none transition-all duration-200 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 focus:shadow-lg ${
      ...
      <div className="mb-4">
        <div className="relative">
          <div className="relative flex items-center">
            <span className="absolute left-0 pl-3.5 text-gray-400 z-10">
              <svg
            ...
            <span
              className={`absolute right-0 pr-3.5 text-gray-400 z-10 ${
                disabled ? 'cursor-not-allowed' : 'cursor-pointer hover:text-gray-600'
              }`}
              onClick={handleToggleCalendar}
            >
              <svg
                className={`fill-current transition-all duration-300 w-4.5 h-4.5 ${
          ...
          {isCalendarOpen && !disabled && (
            <div
              ...
              className="absolute bottom-full mb-2 rounded-xl border border-gray-200 bg-white shadow-2xl z-50 w-full max-w-sm backdrop-blur-sm"
              style={{
            ...
            >
              <div className="p-4">
                <div className="flex items-center justify-between mb-4">
                  <button
                    className="rounded-lg p-2 text-gray-600 hover:bg-gray-100 hover:text-gray-800 transition-all duration-200"
                    onClick={handlePrevMonth}
                  >
                    <svg className="fill-current w-4.5 h-4.5" viewBox="0 0 20 20">
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      className="text-base font-semibold text-gray-800 hover:text-indigo-600 transition-colors duration-200 px-2 py-1 rounded hover:bg-indigo-50"
                ...
                {showYearPicker && (
                  <div className="mb-4 max-h-48 overflow-y-auto rounded-lg bg-gray-50 p-3">
                ...
                <div className="flex justify-end space-x-2 border-t border-gray-100 pt-3">
                  <button
                    className="rounded-lg border border-gray-300 px-4 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all duration-200"
                    onClick={handleCancel}
                  >
                    Cancel
                  </button>
                  <button
                    className="rounded-lg bg-indigo-500 px-4 py-1.5 text-sm font-medium text-white hover:bg-indigo-600 transition-all duration-200 shadow-md"
                    onClick={handleApply}
                  >
                    Apply
                  </button>
